...
from windows.filter_windows import MedianFilterWindow, MaxFilterWindow, MinFilterWindow
...
class ImageEditorApp:
    ...
    def apply_averaging_filter(self):
        """Apply an averaging filter to the current image"""
        if self.cv_image is None:
            messagebox.showwarning("No Image", "Please load an image first.")
            return
            
        result_image = self.image_processor.apply_averaging_filter(self.cv_image)
        self.apply_operation(result_image, "Apply Averaging Filter")
    
    def apply_weighted_averaging_filter(self):
        """Apply a weighted averaging filter to the current image"""
        if self.cv_image is None:
            messagebox.showwarning("No Image", "Please load an image first.")
            return
            
        result_image = self.image_processor.apply_weighted_averaging_filter(self.cv_image)
        self.apply_operation(result_image, "Apply Weighted Averaging Filter")
    
    def apply_gaussian_filter(self):
        """Apply a Gaussian filter to the current image"""
        if self.cv_image is None:
            messagebox.showwarning("No Image", "Please load an image first.")
            return
            
        result_image = self.image_processor.apply_gaussian_filter(self.cv_image)
        self.apply_operation(result_image, "Apply Gaussian Filter")
